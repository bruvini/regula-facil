...
const PacientesUTI = () => {
  ...
  // Log para debug
  console.log('🏥 PacientesUTI - Estado atual:', {
    loading,
    quantidadePacientes: pacientesUTI.length,
    pacientes: pacientesUTI.map(p => ({ id: p.id, nome: p.nome }))
  });

  // Renderizar sempre o componente, mas com informações de debug
  if (loading) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className="text-xs">Debug: {loading ? 'Carregando' : 'Carregado'}</Badge>
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {pacientesUTI.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Verifique no console se há logs de debug para mais informações.
              </p>
            </div>
          ) : (
